...
<div class="container">
  <?php foreach($rows as $c): ?>
    <div class="card">
      <div><cite class="cite-chip" data-cite-id="<?=h($c['key_code'])?>"><?=h($c['key_code'])?></cite></div>
      <p><?=h($c['quote_text'])?></p>
      <div class="small"><?=h($c['source_text'] ?? '')?></div>
      <p><button class="btn primary" onclick="cite('<?=h($c['key_code'])?>')">Цитирай (копира)</button></p>
    </div>
  <?php endforeach; ?>
</div>
</body></html>
